...
Client.REQUESTS = {
  ...
  6: function(address, value) {
    if (typeof value !== 'number') throw new Error('"Write Single Register" expects a \'number\' value');
    return putTwoWord16be(address, value);
  }
};
...
Client.RESPONSES = {
  ...
  // WRITE_SINGLE_REGISTER
  6: function(bufferlist) {
    var rtn = [];
    var binary = Binary(bufferlist)
      .getWord16be('address').end();
    rtn.address = binary.vars.address;
    binary.getWord16be('value').end();
    rtn.value = binary.vars.value;
    return rtn;
  }
};
